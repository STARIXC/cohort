    ...
    $.getJSON("combinedindicators.json", function(result) {
      ...
      for (a = 0; a < result.length; a++) {
        if (result[a].category === currentcohort) {
          ...
          if (isnewrow === 1) {
            row1 =
              "<tr> <td class='col-xs-12' colspan='4'> <div class='control-group'> <label> <font color='red'> <b> * </b> </font> " +
              indicatorname +
              " </label> </div> </td> </tr>";
            count++;
            row2 += row1;
          ...
          row2 +=
            ...
            label +
            " </label> " +
            "<div class='controls'> <input   required='true' onkeypress='return numbers(event);' " +
            isreadonly +
            "  " +
            tabindex +
            ' onblur="' +
...
function getFacilitiesJson() {
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      //$("#facilityname-3").html(data);
      //$("#facilityname-6").html(data);
      $(document).ready(function() {
        $("#facilityname").select2();
       // $("#facilityname-6").select2();
      });
    }
  });
}
